...
# --- Category-level keyword matching (regex, whole speeches) ------------
...
# --- Phrase-level extraction (spaCy, per-speech) -------------------------
...
def plot_top_bar_chart(phrases_df, phrase_col="phrase", count_col="total_count", top_n=10, title=None, year_start=YEAR_START, year_end=YEAR_END):
    """Bar chart of the top N phrases by count."""
    top_df = phrases_df.head(top_n)
    if title is None:
        title = f"Top {top_n} Political Keywords ({year_start}-{year_end})"
